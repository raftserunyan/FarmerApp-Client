...
export const loadHealthStatuses =
	search => async dispatch => {
		try {
			const { data, total } = await HttpService.post(
				'health-status',
				search
			);
...
export const editHealthStatus =
	values => async dispatch => {
		try {
			await HttpService.put(
				`health-status/${values.id}`,
				values
			);
...
export const createHealthStatus =
	values => async dispatch => {
		try {
			const createdHealthStatus = await HttpService.post(
				'health-status',
				values
			);
